...
namespace Problem11
{
    class Program
    {
        ...
        */
        ...
        //Because 1x70x54x71 = 71x54x70x1 we are going to say left/right is horizontal, up/down is vertical
        //As for diagonal the same applies, there is only two unique diagonal directions we are going to search
        //We only need the biggest product, so that is all we are searching and storing
        ...
        static void Main(string[] args)
        {
            ...
            {
                for (var iii = 0; iii <= row.Count - 4; ++iii)
                {
                    var product = row[iii] * row[iii + 1] * row[iii + 2] * row[iii + 3];
                    if (product > biggestProduct) biggestProduct = product;
                }
            }

            //Verticals
            for (var y = 0; y < Numbers.Count - 4; ++y)
                for (var x = 0; x < Numbers.Count; ++x) //Only works because its a square
                {
                    var product = Numbers[y][x] * Numbers[y + 1][x] * Numbers[y + 2][x] * Numbers[y + 3][x];
                    if (product > biggestProduct) biggestProduct = product;
                }

            //45-deg diagonals (0 deg is x axis pointing right) -> Points from bottom left to top right
            for (var y = 3; y < Numbers.Count; ++y)
                for(var x = 0; x <= Numbers.Count - 4; ++x)
                {
                    var product = Numbers[y][x] * Numbers[y - 1][x + 1] * Numbers[y - 2][x + 2] * Numbers[y - 3][x + 3];
                    if (product > biggestProduct) biggestProduct = product;
                }

            //135-deg diagonals  (0 deg is x axis pointing right) -> Points from top left to bottom right
            for (var y = 0; y <= Numbers.Count - 4; ++y)
                for (var x = 0; x <= Numbers.Count - 4; ++x)
                {
                    var product = Numbers[y][x] * Numbers[y + 1][x + 1] * Numbers[y + 2][x + 2] * Numbers[y + 3][x + 3];
